...
class sour_inst(object):
      ...
      def Get_con  (self):
          try:
             ...
             conn = cx_Oracle.connect('dba_username','passwd',dsn_tns)
      ...
      def del_pri  (self):
          ...
          del_sql="""delete from %s.%s where 1=1 """ %(self.db,self.tb)
          for r in rows:
              for (name, value) in zip([r[0]],[r[1]]):
                  dict[name] = value 
          for row1 in self.out_data():
            for r1 in row1:
...
class dest_inst(object):
      def __init__ (self,host,port,dbname,tablename,sid):
         self.host      =host
         self.port      =port
         self.dbname    =dbname
...
def main():
     ...
     dd=dest_inst(dp1,dp2,dp3,dp4,dp5)
     ss.Is_pri()
